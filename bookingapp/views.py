    ...
    if request.user.is_superuser:
        if request.method == 'POST':
            ...
            if start_time == '' or end_time == '' or date  == '' or doctor == '':
                ...
                messages.warning(request, f'Start time and End time is not matched')
            else:
                appointment = AppointmentDetail(start_time= start, end_time = end, date = date, doctor_id = doctormodel)
                appointment.save()
...
def book(request, pk):
    ...
    if request.method == 'POST':
        ...
        if time == '' or prblm == '':
            ...
            messages.warning(request, f'Selected time is not available. It is already booked.')
        else:
            inputtime =  str(time)
            ...
            if stringtime<appstarttime or stringtime>appendtime:
                messages.warning(request, f'Please select the approprite time from '+ stringstarttime+' to '+ stringendtime)
            elif any == True:
                obj = Appointmentlist(user_id = request.user, appointment_id = appointment, time =time, problem_description = prblm)
                obj.save()
                messages.success(request, f'Your appointment at '+time+' with '+appointment.doctor_id.username+' has been booked.')
...
def clients(request):
    if not request.user.is_doctor:
        ...
        appoint_id = AppointmentDetail.objects.filter(doctor_id = request.user).first()
    ...
    if request.user.is_superuser:
        ...
        if request.method == 'POST':
            ...
            if start_time == '' or end_time == '' or date  == '' or doctorid == '':
                ...
                appointlist = Appointmentlist.objects.filter(appointment_id = obj).all()
                appointlist.delete()
                obj.save()
...
def patient_form(request, pk):
    ...
    if request.user.is_doctor:
        if request.method == 'POST':
            ...
            doctorobj = UserModel.objects.filter(username = request.user.username).first()
            if name == '' or contact == '' or problem == '' or medicine == '':
                messages.warning(request, f'Fields cannot be empty')
            else:
                patient = Patient(name = name, contact= contact, problem = problem, medicine = medicine, doctor_id = doctorobj)
                patient.save()
